...
-- ox_target: registers targets on nearby pump entities dynamically.
local oxTargetRegistered = {} -- track which pump entities have targets added

local function registerPumpOxTarget(entity)
    if oxTargetRegistered[entity] then return end
    oxTargetRegistered[entity] = true

    exports.ox_target:addLocalEntity(entity, {
        {
            name = "nd_fuel_grab_nozzle",
        ...
        {
            name = "nd_fuel_return_nozzle",
            icon = "fas fa-undo",
            label = "Return Nozzle",
            distance = config.oxTargetDistance,
            canInteract = function()
                return holdingNozzle and not nearTank and pumpHandle == usedPump
            end,
            onSelect = function()
                LoadAnimDict("anim@am_hold_up@male")
                TaskPlayAnim(ped, "anim@am_hold_up@male", "shoplift_high", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
    ...
    while true do
        ...
        if pump then
            ...
            if config.useOxTarget then
                -- Register ox_target on the pump entity if not already done.
                registerPumpOxTarget(pumpHandle)
            else
                -- Fallback: original 3D text + keybind behaviour.
